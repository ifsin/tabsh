...
static void parse_theme(struct json_object *obj, struct app_theme *t) {
    ...
    if (json_object_object_get_ex(obj, "foreground",    &o)) { strncpy(t->foreground,   json_object_get_string(o), sizeof(t->foreground)    - 1);   t->has_foreground = true; }
    if (json_object_object_get_ex(obj, "background",    &o)) { strncpy(t->background,   json_object_get_string(o), sizeof(t->background)    - 1);   t->has_background = true; }
    if (json_object_object_get_ex(obj, "cursor",        &o)) { strncpy(t->cursor,       json_object_get_string(o), sizeof(t->cursor)        - 1);   t->has_cursor = true; }
    if (json_object_object_get_ex(obj, "cursor_style",  &o)) { strncpy(t->cursor_style, json_object_get_string(o), sizeof(t->cursor_style)  - 1);   t->has_cursor_style = true; }
    if (json_object_object_get_ex(obj, "cursor_blink",  &o)) { t->cursor_blink = json_object_get_boolean(o);                                        t->has_cursor_blink = true; }
    if (json_object_object_get_ex(obj, "font_size",     &o)) { t->font_size = json_object_get_int(o);                                               t->has_font_size = true; }
    if (json_object_object_get_ex(obj, "font_family",   &o)) { strncpy(t->font_family,  json_object_get_string(o), sizeof(t->font_family)   - 1);   t->has_font_family = true; }
    if (json_object_object_get_ex(obj, "palette",       &o)) {
        ...
        for (int i = 0; i < plen && i < 16; i++) {
            ...
            if (pe) { strncpy(t->palette[i], json_object_get_string(pe), sizeof(t->palette[i]) - 1); t->has_palette[i] = true; }
        }
    }
}
...
static int parse_app(struct json_object *obj, struct app_entry *app) {
    ...
    if (json_object_object_get_ex(obj, "args", &o)) {
        ...
        for (int i = 0; i < n && i < MAX_APP_ARGS; i++) {
            ...
            if (a) app->args[app->argc++] = strdup(json_object_get_string(a));
        }
    }
...
static int try_load(const char *path) {
    ...
    if (json_object_object_get_ex(root, "apps", &o)) {
        ...
        for (int i = 0; i < n && cfg->app_count < MAX_APPS; i++) {
            ...
            if (parse_app(app_obj, app) != 0) { json_object_put(root); return -1; }
