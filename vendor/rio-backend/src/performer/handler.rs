...
/// Decode a percent-encoded string (e.g. from OSC 777;cmd payloads).
/// Unknown or malformed sequences are left as-is.
...
pub trait Handler {
    ...
    /// Set or clear the currently running command (OSC 777;cmd).
    /// Empty string means the command finished (prompt returned).
    fn set_current_command(&mut self, _cmd: String) {}
...
impl<U: Handler> Perform for Performer<'_, U> {
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // OSC 777;cmd;<percent-encoded-command> — set/clear the running command.
            b"777" if params.len() >= 2 && params[1] == b"cmd" => {
                let encoded = if params.len() >= 3 {
                    std::str::from_utf8(params[2]).unwrap_or_default()
                } else {
                    ""
                };
                let cmd = percent_decode(encoded);
                self.handler.set_current_command(cmd);
            }
