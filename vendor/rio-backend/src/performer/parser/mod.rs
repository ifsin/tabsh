    ...
    fn decode_codepoints(&mut self, src: &[u8]) {
        ...
        while consumed < src.len() {
            match std::str::from_utf8(&src[consumed..]) {
                Ok(s) => {
                    self.decode_buf.extend(s.chars().map(|c| c as u32));
                    break;
                }
                Err(e) => {
                    ...
                    if valid_up_to > 0 {
                        let valid = unsafe {
                            std::str::from_utf8_unchecked(
                                &src[consumed..consumed + valid_up_to],
                            )
                        };
                        self.decode_buf.extend(valid.chars().map(|c| c as u32));
                    }
                    self.decode_buf.push(0xFFFD);
                    let subpart = maximal_subpart(&src[consumed + valid_up_to..]);
                    consumed += valid_up_to + subpart.max(1);
                }
            }
        }
    }
